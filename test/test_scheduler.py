"""Scheduler tests: admission, batching policy, preemption, finish.

No model is involved anywhere here. The scheduler only moves sequences
between queues and asks the BlockManager for space, so each test drives
step() directly and checks both the returned batch and the block state.
"""
import pytest
...
BS = 4  # block_size used throughout
...
def run_step(sched, batch):
    """Play the engine's part: mark the batch computed and append a token."""
...
def test_first_step_is_a_prefill_batch():
    ...
    assert [s.seq_id for s in batch.seqs] == [1, 2]     # FCFS order
    assert len(sched.waiting) == 0


def test_token_budget_limits_prefill_admission():
    sched = make_scheduler(max_num_batched_tokens=BS)   # room for one prompt
    sched.add_request(make_seq(1))
    sched.add_request(make_seq(2))
    batch = sched.step()
...
def test_block_shortage_limits_prefill_admission():
    sched = make_scheduler(num_blocks=1)                # room for one seq
    sched.add_request(make_seq(1))
    sched.add_request(make_seq(2))
...
def test_head_of_line_blocking_is_strict():
    # A big prompt at the head blocks smaller ones behind it (no skipping).
    ...
    assert [s.seq_id for s in sched.step().seqs] == [1, 2]  # both fit: 5+1=6

    sched = make_scheduler(max_num_batched_tokens=5)
    sched.add_request(make_seq(1, prompt_len=5))
    sched.add_request(make_seq(2, prompt_len=1))        # fits alone, but waits
    assert [s.seq_id for s in sched.step().seqs] == [1]
...
def test_prompt_larger_than_whole_cache_rejected():
    sched = make_scheduler(num_blocks=2)                # 2*BS tokens total
...
def test_prefill_first_runs_new_requests_before_decodes():
    ...
    sched.add_request(make_seq(2))
    batch = sched.step()
    assert batch.is_prefill                             # prefill wins
...
def test_decode_first_keeps_running_stream_steady():
    ...
    sched.add_request(make_seq(2))
    batch = sched.step()
    assert not batch.is_prefill                         # decode wins
    assert [s.seq_id for s in batch.seqs] == [1]
...
def test_prefill_waits_when_blocks_short_but_decode_continues():
    # Seq 2 needs 3 blocks and only 2 are free, so it waits; the running
    # sequence still gets its decode step rather than idling.
    sched = make_scheduler(num_blocks=3)
    sched.add_request(make_seq(1, prompt_len=BS))       # 1 block
...
def test_decode_preemption_evicts_youngest_and_requeues():
    # Two sequences, cache full after prefills. The next decode needs one
    # more block, so the youngest (seq 2) is evicted.
    sched = make_scheduler(num_blocks=3)
    sched.add_request(make_seq(1, prompt_len=BS + 1))   # 2 blocks
    sched.add_request(make_seq(2, prompt_len=BS))       # 1 block
    run_step(sched, sched.step())
    assert sched.block_manager.num_free_blocks() == 0

    batch = sched.step()                                # decode: needs 2 slots
    assert not batch.is_prefill
    assert [s.seq_id for s in batch.seqs] == [1]        # seq 2 evicted

    victim = sched.waiting[0]
    assert victim.seq_id == 2
    assert victim.status is SeqStatus.PREEMPTED
    assert victim.num_computed_tokens == 0              # KV is gone
    assert victim.num_tokens == BS + 1                  # tokens survive
...
def test_preempted_sequence_reprefills_its_whole_context():
    ...
    run_step(sched, sched.step())
    sched.step()                                        # preempts seq 2
    sched.finish(sched.running[0], now=1.0)             # free seq 1's blocks

    batch = sched.step()                                # seq 2 readmitted
    assert batch.is_prefill
    assert [s.seq_id for s in batch.seqs] == [2]
    # Re-prefill covers prompt + generated token: 2 blocks, not 1.
...
def test_preemption_goes_to_front_of_waiting_queue():
    ...
    sched.add_request(make_seq(2, prompt_len=BS))
    run_step(sched, sched.step())
    sched.add_request(make_seq(3))                      # arrives before eviction
    sched.step()                                        # preempts seq 2
    assert [s.seq_id for s in sched.waiting] == [2, 3]  # victim jumps the queue
...
def test_finish_frees_blocks_for_new_requests():
    ...
    sched.add_request(make_seq(1, prompt_len=BS))       # 1 block
    run_step(sched, sched.step())
    sched.add_request(make_seq(2, prompt_len=2 * BS))   # needs 2 blocks
    assert not sched.step().is_prefill                  # only 1 free: no room
