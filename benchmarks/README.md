...
**Online serving** (`online.py`) - requests arrive over time on a Poisson
...
One online point at a time:

```bash
uv run python benchmarks/online.py --request-rate 8 --num-prompts 200 \
    --engine nanoserve --model-path Qwen/Qwen3-0.6B --out results/one.json
```
